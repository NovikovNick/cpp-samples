...
int net_resolve_address(ADDRESS *addr, char *host, int port) {
  ...
  if (hp) *(int *)&addr->sin_addr.s_addr = *(int *)hp->h_addr_list[0];
  return 0;
}
...
static void *net_thread(void *param) {
  ...
  if (link->host) net_resolve_address(link->addr, link->host, link->port);

  printf("resolved to %s\n", net_to_string(link->addr));

  if ((link->sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) goto cleanup;
  ...
  while (link != NULL) {
    ...
    len = recvfrom(link->sock, buf, buf_size, 0, (struct sockaddr *)&addr,
                   &addr_size);
...
int net_open_link(LINK *link) {
  ...
  CreateThread((LPSECURITY_ATTRIBUTES)NULL, 0,
               (LPTHREAD_START_ROUTINE)net_thread, link, 0, &link->thread);
#else
...
int net_send(LINK *link, char *buf, int len) {
  ...
  sendto(link->sock, buf, len, 0, (struct sockaddr *)link->addr,
...
#define stun_write_uint(m, attr, value) \
  w16(m, attr);                         \
  w16(m, 4);                            \
  w32(m, value);
...
int stun_update(STUN *stun) {
  ...
  switch (stun->state) {
    case 0:  // starting
      if (stun->link->started) {
        int i;
        stun->timeout = 4;
        for (i = 0; i < 4; i++) stun->results[i] = 0;
        state = 1;
      }
      break;
    ...
    case 2:  // wait result
      if (time(0) - stun->send_time > stun->timeout) state = 3;
      if (stun->received) state = 4;
      break;

    case 3:
      printf("[timeout]\n");

      if (stun->test == 1) stun->finished = 1;

      if (stun->test == 2 && !stun->nat) stun->finished = 1;
    ...
    case 4:
      ...
      if (stun->test > 4) {
        stun->mappedports[stun->test - 5] =
            ntohs(stun->mapped_address.sin_port);
      }
  ...
  if (stun->test == 9) stun->finished = 1;
...
void net_recv(char *buf, int len) { stun_recv(&m_stun, buf, len); }
...
int main(int argc, char **argv) {
  ...
  while (stun_update(stun)) Sleep(5);

  // make a mask for tests, i.e. 1,0,1,1 == 0x1011
  int i, mask = 0;
  for (i = 0; i < 4; i++) mask |= stun->results[i] << (3 - i) * 4;
  ...
  if (stun->nat) printf("type: %s\n", type);

  printf("mapped ports: ");
  for (i = 0; i < 4; i++) printf("%d ", stun->mappedports[i]);
